...
public class GenSparkUtils {
        ...
        for (Operator<?> root : roots) {
            ...
            for (Operator<?> op : opQueue) {
                ...
                if (op instanceof FileSinkOperator) {
                    ...
                    if (op instanceof SparkPartitionPruningSinkOperator) {
   ...
   */
    private static boolean groupByNeedParLevelOrder(ReduceSinkOperator reduceSinkOperator) {
        ...
        if (children != null && children.size() == 1 && children.get(0) instanceof GroupByOperator) {
            ...
            if (isSame(reduceSinkOperator.getConf().getKeyCols(), reduceSinkOperator.getConf().getPartitionCols()) && reduceSinkOperator.getConf().getKeyCols().size() == child.getConf().getKeys().size()) {
                return false;
            }
        }
        return true;
   ...
   */
    ...
    public static <T extends java.lang.Object> T getChildOperator(Operator<?> op, Class<T> klazz) throws SemanticException {
